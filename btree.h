...
public:
  ...
  ERROR_T InsertHelper(const SIZE_T &node,
             const KEY_T &key,
             const VALUE_T &value,
             SIZE_T &newDiskBlock,
  ...
  ERROR_T DeleteHelper(const SIZE_T &node, const KEY_T &key); 
  // return zero on success
  // return ERROR_NONEXISTENT  if the key doesn't exist
  ERROR_T Lookup(const KEY_T &key, VALUE_T &value);

  ERROR_T RangeQuery(const KEY_T &minKey, const KEY_T &maxKey, std::vector<VALUE_T> &values);
  ...
  ERROR_T InOrderCheck(const SIZE_T &node, std::vector<KEY_T> &allData, std::vector<VALUE_T> &values) const;
  ERROR_T GetHeight(const SIZE_T &node, int &height)const;
  ERROR_T UseRatioCheck(const SIZE_T &node)const;
  // Display tree
